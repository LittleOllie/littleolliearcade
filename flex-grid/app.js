  ...
  return gridImgLimit(
    () =>
      new Promise((resolve, reject) => {
        ...
        // ✅ only needs to exist HERE (and in loadImgNoLimit)
        imgEl.src = "";
        imgEl.src = src;
      })
  );
...
function loadImgNoLimit(imgEl, src) {
  return new Promise((resolve, reject) => {
    ...
    // ✅ only needs to exist HERE (and in loadImgWithLimiter)
    imgEl.src = "";
    imgEl.src = src;
  });
}
...
// 1) proxy-first (limited)
// 2) if not IPFS and direct https, try direct
...
async function loadTileImage(tile, img, rawUrl) {
  ...
  if (!directNormalized) return markMissing(tile, img, rawUrl);
...
(function bindEvents() {
  ...
  function retryMissingTiles() {
    ...
    tiles.forEach((tile) => {
      ...
      loadImgWithLimiter(img, gridProxyUrl(src)).catch(() => {});
    });
